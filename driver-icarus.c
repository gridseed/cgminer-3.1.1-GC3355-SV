...
//#define ICARUS_READ_COUNT_TIMING	(50 * TIME_FACTOR)
...
static void icarus_init(int fd)
{
...
static bool icarus_detect_one(const char *devpath)
{
	...
	unsigned char ob_bin[64], nonce_bin[ICARUS_READ_SIZE], my_bin[52];
	char *nonce_hex;
	...
	applog(LOG_DEBUG, "Icarus Detect: Attempting to open %s on %d", devpath, baud);
	...
	memcpy(my_bin, "\x55\xaa\x0f\x00\xa0\x87\x01\x00", 8);
				...
				__maybe_unused int64_t max_nonce)
{
	...
	/* Added by faster */
    memset(my_bin, 0, sizeof(my_bin));
	memcpy(my_bin, "\x55\xaa\x0f\x00", 4);
    memcpy(my_bin+8, ob_bin, 32);
    memcpy(my_bin+40, ob_bin+52, 12);
	/* End */
...
#endif
	// change by faster
	ret = icarus_write(fd, my_bin, sizeof(my_bin));
	...
	memset(nonce_bin, 0, sizeof(nonce_bin));
	info = icarus_info[icarus->device_id];
	ret = icarus_gets(nonce_bin, fd, &tv_finish, thr, info->read_count/4);
